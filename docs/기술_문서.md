...
[Fabric Mod(인게임, 메인)]  ──┐
                              ├─▶ FastAPI (API · SSE)
[웹 챗봇(Streamlit, 검증용)] ─┘        │
                                       ├─▶ LangGraph (11노드 Agentic 워크플로우)
                                       │       ├─ MySQL  (세션 · 메시지 · 코칭 진척도)
                                       │       ├─ Qdrant (Vector · 위키 RAG)
                                       │       ├─ knowledge/ (결정론 레시피·확정 사실)
                                       │       ├─ ko.minecraft.wiki API (키 없는 웹검색 폴백)
...
START → load_state → analyze ─(domain)─┬─ general·out_of_scope ──────────────────────────────→ respond ─┐
                                        │                                                                 │
                                        └─ minecraft → resolve_goal → clarify ─(정보 충분?)─┬─ 부족 → ask ─┤
                                                                                            │             │
...
모든 종료 경로는 `persist_state`를 거쳐 진척도를 저장한다.
...
| `check_materials` | `agents/material_checker.py` | 제작 목표가 있으면 결정론으로 부족 재료·채굴 티어 계산 + 3×3 격자 첨부 |
...
- `minecraft` → temperature 0.2 + `streaming=True`, 아래 블록을 모두 프롬프트에 주입
  - `[추천 목표]`(막연형 제안) · `[진행 상황]`(새로 얻은 재료) · `[목표 진행]`(완료 단계·다음 단계) · `[현재 상태]`(시간·체력) · `[현재 인벤토리]` · `[제작 분석]`(부족 재료) · `[확정 규칙]` · 참고 위키
...
> 기존 `app/knowledge/minecraft_facts.py`(MINING_REQ·PICKAXE_LEVEL·ACQUIRE·ALIASES·`ITEM_ID_TO_KO`(72)·`item_ko()`)와 `facts.py:lookup_facts()`의 **확정 사실 계층**도 유지되어 `[확정 규칙]` 블록으로 함께 주입된다(참고 위키보다 우선).
